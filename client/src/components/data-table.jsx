...
const TagsCell = React.memo(({ tags }) => (
  <div className="flex flex-wrap align-center items-center gap-1 w-fit">
    {tags?.slice(0, 2).map((tag, index) => (
      <Badge key={index} variant="secondary" className="text-xs text-right">
        {tag}
      </Badge>
    ))}
  </div>
...
// Memoized Company Cell Component
const CompanyCell = React.memo(({ company }) => (
  <div className="flex flex-wrap gap-1 max-w-48">
    {company?.slice(0, 2).map((tag, index) => (
...
const useFilters = (data) => {
  ...
  const companies = React.useMemo(() => {
    const uniqueCompanies = data.flatMap((item) => item.company || [])
    return Array.from(new Set(uniqueCompanies)).sort()
  }, [data])
  ...
  const filteredData = React.useMemo(() => {
    return data.filter((item) => {
      ...
      const matchesCompany = companyFilter === "all" || (item.company && item.company.includes(companyFilter))

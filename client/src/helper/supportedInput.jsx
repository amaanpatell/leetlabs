...
export default function SupportedInputs({ inputTypes }) {
  return (
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-300">
            Notebook LM works with multiple content formats to give you maximum flexibility
          </p>
        </div>
                    ...
                    <ol className="space-y-2">
                      {type.steps.map((step, idx) => (
                        <li key={idx} className="text-gray-400 text-sm flex items-start gap-3">
                          <span className="shrink-0 w-5 h-5 rounded-full bg-blue-600 text-white text-xs flex items-center justify-center mt-0.5">
                            {idx + 1}
                          </span>
                          {step}
                        </li>
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      {type.tips.map((tip, idx) => (
                        <li key={idx} className="text-gray-400 text-sm flex items-start gap-2">
                          <span className="w-3 h-3 mt-1 rounded-full bg-yellow-400/70 shrink-0" />
                          {tip}
                        </li>
                      ))}
                    </ul>
